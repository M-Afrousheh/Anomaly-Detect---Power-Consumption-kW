...
def main():
    ...
    # Flag only once when a run reaches MIN_CONSECUTIVE
    s = pd.Series(is_point)
    grp = (s != s.shift(1)).cumsum()
    run_len = s.groupby(grp).cumcount() + 1
    ...
    ax.plot(out.index, out["demand"], color="tab:blue", label="actual demand (kW)")
    ax.plot(out.index, out["y_pred"], color="tab:orange", alpha=0.8, label="predicted demand (kW)")
    ax.scatter(
        out.index[out["is_anom"]],
        out["demand"][out["is_anom"]],
        color="red",
        s=14,
        label="anomaly",
        zorder=3,
    ...
    ax.text(0.01, 0.02, metrics_text, transform=ax.transAxes, fontsize=9, va="bottom")
    fig.tight_layout()
    fig.savefig(OUTPUT_DIR / "demand_actual_vs_predicted.png", dpi=150)
    ...
    # Correlation bar chart (features vs demand)
    corr = df[feature_cols + [DEMAND_COL]].corr()[DEMAND_COL].drop(DEMAND_COL)
    corr = corr.sort_values(key=lambda s: s.abs(), ascending=False)
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.barh(corr.index, corr.values, color="tab:green")
    ax.invert_yaxis()
    ...
    max_run = int(pd.Series(is_point).groupby((pd.Series(is_point) != pd.Series(is_point).shift(1)).cumsum()).sum().max())
